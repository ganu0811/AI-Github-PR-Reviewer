...
'''Finding summary line will convert one finding into a readable text, as currently 
...
'''Build Summary will combine all the findings into a single comment'''

def _build_summary_(findings:list)->str:
...
async def post_review(request: ReviewRequest):
    ...
    summary = _build_summary_(request.findings)
    ...
    async with httpx.AsyncClient() as client:
        ...
        '''sometimes github rejects the inline comments
        due to line mismatching or some other error.
        Thats why we are initiating again the inline comments'''
