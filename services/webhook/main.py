...
settings = Settings()
engine = create_async_engine(settings.database_url) # connecting to database. It will be handled async engine of sqlalchemy
AsyncSessionLocal = sessionmaker(engine,class_=AsyncSession, expire_on_commit=False) # creating a session or talking to database
...
async def receive_event(request: Request):
    ...
    async with AsyncSession() as session:
        ...
        if result.scalar_one_or_none():
            return{"status": "already_preprocessing"}
